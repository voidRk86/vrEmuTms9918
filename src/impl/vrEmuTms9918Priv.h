...
#define T_R1_INTERLEAVE_COLOR   0x04
  ...
  else
  {
    ...
    if (data & 0x80) /* register */
    {
      if ((data & 0x40) == 0) // Was 0x78, but we allow 64 registers = 0x40
      {
        ...
        if ((data & 0x7F) == TMS_REG_1 /* && tmsMode(tms9918) == TMS_MODE_TEXT80_8 */ && (tms9918->regWriteStage0Value & T_R1_INTERLEAVE_COLOR) == 0 )
    ...
    else /* address */
    {
      ...
      if (tmsMode(tms9918) == TMS_MODE_TEXT80_8 && (TMS_REGISTER(tms9918, TMS_REG_1) & T_R1_INTERLEAVE_COLOR) != 0)
